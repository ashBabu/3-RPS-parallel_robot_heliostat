...
% KJ = -0.0485; % distance between the mirror center and the top platform center
KJ = -0.25;
...
gamma = 135; % the angle by which the x_b is inclined towards X (East)
Rot_gam=[ cosd(gamma) -sind(gamma) 0;
          sind(gamma)  cosd(gamma) 0;
              0            0       1]; 
ang = 90;
[Rot,RotZ,eta,Zeta]=Rotmatrix_AzEl(elevation(v:w)',azimuth(v:w)',ang);   % Using the normal to find the rotmat

Rot=RotZ;
for i=1:length(Rot)
    O1G(:,i) = O1M+KJ*Rot(:,3,i);
end
%% Leg lengths
figure(1)
rb=0.50;  % circum radius of the base equilateral triangle, B1B2/sqrt(3) in meters
rp=0.50;  % circum radius of the platform,P1P2/sqrt(3)in meters
...
% open(writerObj);
OM = OO1 + Rot_gam*O1M; % to mirror centre, M, in gcs
MP = (OP-OM)/norm((OP-OM),2); % wrt gcs reflected ray
...
%     normal=plot3([OM(1,1) OM(1,1)+kon*Rot(1,3,i)],[OM(2,1) OM(2,1)+kon*Rot(2,3,i)],[OM(3,1) OM(3,1)+kon*Rot(3,3,i)],'r','LineWidth',2);
...
%     sunposition=plot3([OM(1,1) OM(1,1)+kons*xs(zz)],[OM(2,1) OM(2,1)+kons*ys(zz)],[OM(3,1) OM(3,1)+kons*zs(zz)],'k','LineWidth',2);   % position of the sun at every time_incre looking from plate coordinte system
    ...
    %%
    kjs =.5;
%     plot3([OM(1,1) OM(1,1)+kjs*MP(1,1)],[OM(2,1) OM(2,1)+kjs*MP(2,1)],[OM(3,1) OM(3,1)+kjs*MP(3,1)],'b','LineWidth',2);
...
%     plot3([a a],[b b],[0 c],'b');
    ...
    mg = .7;
        axis([OO1(1)-mg OO1(1)+mg OO1(2)-mg OO1(2)+mg 0 2.5]);  
%         axis([70 71.50 70 72 0 2.5]); 
...
%     axis tight
    view(45,45);
    grid on
    xlabel('X, East','Rotation',-35)
    ylabel('Y, North ','Rotation',40)
    zlabel('Z, Zenith')
    hold off
    M(i)=getframe(gcf);
...
%% Using fsolve to find the rotation matrix for Az-El case
