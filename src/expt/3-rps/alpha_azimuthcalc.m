...
%% Method 1
% a = -cosd(delta).*sind(HRA)./cosd(elevation);
% for i=1:length(HRA)
%     if (cosd(HRA(i)) >= tand(delta)/tand(latitude))
%         azimuth(i) = 180-asind(a(i));
%     else
%         azimuth(i) = 360+ asind(a(i));
%     end
% end
%% Method 2
% a1=sind(delta)*cosd(latitude);
...
%%  Method 3
% a1=sind(delta)*cosd(latitude);
% a2=cosd(delta)*sind(latitude).*cosd(HRA);
% a3 = -cosd(delta).*sind(HRA);
% azimuth = atan2d(a3,(a1-a2));
